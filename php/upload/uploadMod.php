<?php
    ...
    $sql = "SELECT * FROM categoryBoard WHERE userMemberID = '$userMemberID' AND categgoryBoardID = '$categgoryBoardID'";
    
    $result = $connect -> query($sql);
    $info = $result -> fetch_array(MYSQLI_ASSOC);
    
    $tagSql = "SELECT categgoryTag as Tag FROM categoryTag WHERE categgoryBoardID = '$categgoryBoardID'";
    $tagResult = $connect -> query($tagSql);
    $tagInfo = $tagResult -> fetch_array(MYSQLI_ASSOC);
?>
<!DOCTYPE html>
<html lang="ko">
    <head>
    ...
    <body>
        ...
        <main id="main">
            <section id="upload" class="uplaod__wrap">
                    <form action="uploadModSave.php" name="upload" method="post" enctype="multipart/form-data">
                        <div class="upload__inner container">
                            <div class="upload__title">
                                <fieldset>
                                    <input id="inputHidenID" name="inputHidenID" type="hidden" value="<?=$categgoryBoardID?>">
                                    <label for="uploadTitle" class="blind">제목</label>
                                    <input type="text" id="uploadTitle" class="uploadTitle" name="uploadTitle" placeholder="제목을 입력해주세요." value="<?=$info['categgoryTitle']?>" />
                                </fieldset>
                            </div>
                            ...
                            <div class="upload__write">
                                <fieldset>
                                    ...
                                    <textarea name="uploadWrite" id="uploadWrite" class="uploadWrite" rows="20" placeholder="내용을 입력해주세요."><?=$info['categgoryContents']?></textarea>
                                </fieldset>
                            </div>
                            ...
                            <div class="upload__image">
                                ...
                                <span class="upload__image__desc" id="upload__image__desc"><?=$info['categgoryPhoto']?></span>
                            </div>
                            ...
                            <tags class="tagify" tabindex="-1">
                                ...
                                ?>
                                <tag title="<?=$tag['Tag']?>" contenteditable="false" spellcheck="false" tabindex="-1" class="tagify__tag tag__mod" value="<?=$tag['Tag']?>">
                                    <x title="" class="tagify__tag__removeBtn" role="button" aria-label="remove tag"></x>
                                    <div>
                                        <span class="tagify__tag-text"><?=$tag['Tag']?></span>
                                    </div>
                                </tag>
                                <?php } ?>
                            </tags>
        ...
        <script>
                ...
                const removeTag = document.querySelectorAll(".tagify__tag__removeBtn");
                removeTag.forEach((e)=>{
                ...
                function uploadChecks() {
                    ...
                    uploadTags.forEach((e,i) => {
                        uploadTagBox.push(e.getAttribute('title')+'@^@&@!');
                    });
                    ...
                    const uniqueArr = uploadTagBox.filter((element, index) => {
                            return uploadTagBox.indexOf(element) === index;
                    });

                    uploadTagInput.value = uniqueArr.join('');
